...
2. [Задание разработчику: интерфейс и backend](#section-01)
3. [Задание специалисту по безопасности: проверка реализации и БД](#section-02)
4. [00. Объём продукта и правила приоритизации](#section-03)
5. [01. Роли, предметная модель и пользовательские потоки](#section-04)
6. [02. Дизайн-система и визуальное направление](#section-05)
7. [03. Кабинет руководителя: страницы и действия](#section-06)
8. [04. Кабинет сотрудника и прохождение](#section-07)
9. [05. Кабинет администратора](#section-08)
10. [06. Архитектура и организация кода](#section-09)
11. [07. PostgreSQL: сущности и ограничения](#section-10)
12. [08. HTTP API и правила взаимодействия](#section-11)
13. [09. Методики, подсчёт и аналитическое заключение](#section-12)
14. [10. Безопасность, доступ и обращение с данными](#section-13)
15. [11. Ретроспективная проверка и слепой режим](#section-14)
16. [12. Приёмка, производительность и эксплуатация](#section-15)
17. [Каталог визуальных референсов](#section-16)
18. [Roadmap разработки](#section-17)
19. [Связь страниц и задач](#section-18)
20. [База знаний команды](#section-19)
21. [Реестр решений (ADR)](#section-20)
22. [Открытые вопросы и точные границы блокировок](#section-21)
23. [Словарь](#section-22)
24. [Совместная работа людей и агентов](#section-23)
25. [Передача работы](#section-24)
26. [История изменений](#section-25)
27. [Источники и границы подтверждения](#section-26)
28. [Инструкции для разработки](#section-27)
...
*Исходный модуль: `TASKS_ROLE2_FRONTEND.md`*
...
*Исходный модуль: `TASKS_ROLE1_ARCH_SECURITY_DEVOPS.md`*
...
> Реализованная модель 16–17.09. Новые требования к ролям и доступам на следующий этап заданы в [задании разработчику от 22.09](TASKS_ROLE2_FRONTEND.md); при расхождении следовать новому ТЗ.
...
> Исходный ссылочный путь. Новое требование к личному кабинету сотрудника — в [задании разработчику](TASKS_ROLE2_FRONTEND.md).
...
> Исходная модель platform admin. Новый технический администратор не создаёт организацию и не редактирует методики/сценарии; см. [задание разработчику](TASKS_ROLE2_FRONTEND.md) и [проверку безопасности](TASKS_ROLE1_ARCH_SECURITY_DEVOPS.md).
...
- [x] **D-08 · Подготовить два рабочих задания: разработчику и специалисту по безопасности** — Готово · P0
  - Зависимости: D-07.
  - Спецификация: [TASKS_ROLE2_FRONTEND.md](TASKS_ROLE2_FRONTEND.md), [TASKS_ROLE1_ARCH_SECURITY_DEVOPS.md](TASKS_ROLE1_ARCH_SECURITY_DEVOPS.md).
  - Приёмка: Разработчик видит полный путь frontend, backend и БД; Специалист по безопасности видит последовательность проверки реализации и полей БД; Старые и новые правила доступа ясно разведены.
  - Подтверждение: Два документа TASKS_ROLE2_FRONTEND.md и TASKS_ROLE1_ARCH_SECURITY_DEVOPS.md содержат задачи, порядок работы и приёмку. Документация и ссылки проверены; код новых функций ещё не реализован.
  - Проверка: `python3 scripts/build_spec.py && python3 scripts/check_spec.py`.
...
- [x] **F-07 · Подготовить synthetic factories, seeds и test environment** — Готово · P0
  ...
  - Подтверждение: 22.09.2026: в локальной dev-БД создано 6 синтетических черновиков назначений с попытками в двух demo-организациях; повторный запуск создал 0, обнаружил 6. Пароли не меняются. Код: packages/database/src/cli/seed-test-data.ts; команды добавлены в package.json и packages/database/package.json; документация: README.md, docs/knowledge/HANDOFF.md, docs/knowledge/CHANGELOG.md.
  - Проверка: `npm run db:seed:test-data`.

- [ ] **F-08 · Bootstrap пространства, подразделения, права и SMTP** — Не начато · P0
  - Зависимости: D-08.
  - Спецификация: [TASKS_ROLE2_FRONTEND.md](TASKS_ROLE2_FRONTEND.md), [TASKS_ROLE1_ARCH_SECURITY_DEVOPS.md](TASKS_ROLE1_ARCH_SECURITY_DEVOPS.md).
  - Приёмка: Создание организации доступно только bootstrap, редакторы контента закрыты техническому администратору; Подразделения и членства ограничивают доступ на сервере; Статусы отправки и активации разделены.
...
- [ ] **M-08 · Кабинет руководителя подразделения, конструктор и заключение** — Не начато · P0
  - Зависимости: F-08.
  - Спецификация: [TASKS_ROLE2_FRONTEND.md](TASKS_ROLE2_FRONTEND.md).
  - Приёмка: Руководитель видит только свою команду; Конструктор отделяет качественные вопросы от scoring; Заключение даёт поэтапную обратную связь и фиксацию решения.
...
- [ ] **E-07 · Личный кабинет сотрудника и собственные оценки** — Не начато · P0
  - Зависимости: F-08.
  - Спецификация: [TASKS_ROLE2_FRONTEND.md](TASKS_ROLE2_FRONTEND.md).
  - Приёмка: После входа доступны Главная, Оценки и Настройки; Чужие назначения не открываются; Условия участия объясняют видимость ответов.
...
- [ ] **Q-10 · Проверка новых ролей, SMTP, перевода и ответов** — Не начато · P0
  - Зависимости: F-08, M-08, E-07.
  - Спецификация: [TASKS_ROLE2_FRONTEND.md](TASKS_ROLE2_FRONTEND.md), [TASKS_ROLE1_ARCH_SECURITY_DEVOPS.md](TASKS_ROLE1_ARCH_SECURITY_DEVOPS.md).
  - Приёмка: Негативные API-тесты закрывают чужие данные; Проверены токены, SMTP, сессии, перевод и аудит; Четыре сквозных сценария пройдены на синтетике.
...
- [Задание разработчику](TASKS_ROLE2_FRONTEND.md) и [проверка безопасности](TASKS_ROLE1_ARCH_SECURITY_DEVOPS.md) — целевое поведение следующего этапа; код ещё не приведён к нему.
...
## Новая модель ролей и задания команде — 22.09.2026

По уточнению владельца продукта подготовлены два рабочих документа: [задание разработчику frontend и backend](TASKS_ROLE2_FRONTEND.md) и [задание специалисту по безопасности](TASKS_ROLE1_ARCH_SECURITY_DEVOPS.md). Это документация будущей реализации; код текущих `/admin/methods`, `/admin/scenarios`, `POST /admin/organizations`, `/app/settings` и ссылочного входа сотрудника пока не изменён. Новые ADR-037…040 фиксируют отличия от прежнего ТЗ. Следующая конкретная задача: разработчику согласовать четыре правила из своего задания, затем начать API-контракты, миграции и экраны; специалисту по безопасности проверить схему и готовый PR.

## Синтетическое наполнение — 22.09.2026

В ветке `codex/synthetic-test-data` добавлен `npm run db:seed:test-data`: шесть черновиков назначений с попытками для двух demo-организаций. Скрипт требует базовый `db:seed:demo`, не меняет пароли и при повторном запуске не создаёт дубликатов. Локальная dev-БД наполнена: первый запуск создал 6, второй нашёл все 6. Проверка: `npm run typecheck --workspace @context/database` и два запуска seed. Следующая задача при необходимости: добавить синтетические случаи на других этапах прохождения через штатные API, чтобы сохранить инварианты состояний.
...
## 22.09.2026 — ТЗ новой модели ролей

- Подготовлены два рабочих задания: разработчику всего продукта (frontend и backend) и специалисту по безопасности для проверки реализации и БД.
- Отмечены замены ADR-007/008/009/015 и новые решения ADR-037…040. Реализация этих требований ещё не выполнена.

# 22.09.2026 — синтетические назначения

- Добавлен отдельный идемпотентный seed шести черновиков назначений в двух demo-организациях. Пароли учётных записей не меняются.
- Локальная dev-БД наполнена; повторный запуск подтвердил отсутствие дублей.
